...
interface ConflictsBoxProps {
  conflicts: ConflictItem[];
  source?: string;
  target?: string;
}

export default function ConflictsBox({ conflicts, source, target }: ConflictsBoxProps) {
    ...
    <div className="space-y-6 ">
      ...
      <div className="space-y-4">
        {conflicts?.map((conflict, idx) => (
          <div key={idx} className="border border-border/50 rounded overflow-hidden shadow-sm bg-background">
              ...
              {conflict.changes && conflict.changes.length > 0 && (
                <div className="divide-y divide-border/5">
                  <div className="px-3 py-2 bg-muted/5 border-b border-border/5">
                    <div className="text-[10px] uppercase font-bold text-muted-foreground/50">
                      Change Hashes ({conflict.changes.length})
                    </div>
                  </div>
              ...
              {!conflict.content && !conflict.contentA && !conflict.contentB &&
                ...
                <div className="p-6 text-center">
                  <p className="text-muted-foreground text-xs italic">
                    No additional details available for this conflict.
                  </p>
                </div>
              )}
            </div>
